...
describe('parser', () => {
  ...
  describe('single command', () => {
    ...
    it('should parse single command with negated boolean option', async () => {
      ...
      expect(await cli.run(['dev', '--flag=true'])).toMatchInlineSnapshot(
        `true`
      );
      ...
      expect(await cli.run(['dev', '--flag=no'])).toMatchInlineSnapshot(
        `false`
      );
      expect(await cli.run(['dev', '--flag=n'])).toMatchInlineSnapshot(`false`);
      expect(await cli.run(['dev', '--flag=off'])).toMatchInlineSnapshot(
        `false`
      );
      expect(await cli.run(['dev', '--flag=123'])).toMatchInlineSnapshot(
        `true`
      );
      expect(await cli.run(['dev', '--no-flag'])).toMatchInlineSnapshot(
        `false`
      );
      ...
      expect(await cli.run(['dev', '--no-flag=f'])).toMatchInlineSnapshot(
        `true`
      );
      expect(await cli.run(['dev', '--no-flag=no'])).toMatchInlineSnapshot(
        `true`
      );
      expect(await cli.run(['dev', '--no-flag=n'])).toMatchInlineSnapshot(
        `true`
      );
    ...
    it('should parse single command with sub command', async () => {
      ...
      `);
      expect(
        await cli.run(['cmd1', 'XLor', 'gdx', 'xnf', '414', '--flag=false'])
      ).toMatchInlineSnapshot(`
        {
          "cmd2": "XLor",
          "cmd3": "gdx",
    ...
    it('should parse single command with unknown argument', async () => {
      const cli = new Breadc('cli');
  ...
  describe('unknown options', () => {
    it('should not parse with unknown options', async () => {
      // TODO: throw error
      {
        const cli = new Breadc('cli').allowUnknownOptions(false);
